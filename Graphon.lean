...
* `Graphon.RelFixingAlgebra` — R4 converse piece 2a (#107): the **law-independent** factor-algebra layer — `SortwiseFixing` (the `A`-fixing stabilizer of finitely supported sortwise permutations, closed under `1`/`*`/`⁻¹`/conjugation), the raw `RelStructure.fixingAlgebra` (events invariant under the `A`-fixing group; deliberately *not* "generated by relations inside `A`", which loses hidden vertex information), monotonicity, `fixingAlgebra_empty = invariantAlgebra` (near-definitional), and the **transport equality** `fixingAlgebra_comap_relabel : comap (relabel σ) (fixingAlgebra A) = fixingAlgebra (image σ A)` via stabilizer conjugation — no completions, no law; conditional independence under a dissociated law is its own later PR
* `Graphon.RelFixingCondIndep` — R4 converse piece 2b (#107), in progress: the conditional-independence engine behind `condIndep_fixingAlgebra` (Austin arXiv:0801.1698 Lemma 3.11 / Proposition 3.12; Kallenberg Lemma 7.6). Currently the generic conditional-expectation tools — the `L²`-energy squeeze, measure-preserving-map transport of conditional expectation, and the tail-property engine (a.e.-fixing + comap-pullback ⟹ equal conditional expectations) — plus the private window (polling) algebras with their coordinate-join characterization, arbitrary-permutation window transport, and measure preservation under the exchangeable law. All declarations `private`; the public `condIndep_fixingAlgebra` (for **every** exchangeable law — dissociation is a corollary) lands with the polling-tail bridge in the next layer
