...
* `Graphon.RelRankAlgebra` — R4 converse piece 3 (#107): the **lower-rank conditioning algebra** `lowerRankAlgebra n = ⨆ A, ⨆ (_ : A.card < n), fixingAlgebra A`, the single global factor the rankwise relative-independence theorem conditions on. It has to be one global factor rather than a chain or a per-pair conditioning: pairwise conditional independence does not imply mutual, and conditional independence is not preserved under enlarging the conditioning algebra, so `condIndep_fixingAlgebra` is infrastructure and a two-set regression check rather than the proof engine. Law-free: order theory only — monotone in the rank bound, below the ambient algebra, containing each low-rank fixing algebra, `⊥` at rank `0`, and `invariantAlgebra` at rank `1`, the last being the recursion's base and the reason no `NoNullary` hypothesis is needed
