...
Three remaining \texttt{sorry} declarations, driven by two main missing
mathematical inputs (no custom axioms are introduced):
\begin{enumerate}
  ...
  \item \textbf{Algebraic determination} for $k \ge 2$
    (\texttt{matrix\_quotient\_of\_weightedHomSum\_eq}, positive-weight case):
  ...
  \item \textbf{Algebraic determination axiom}
    (\texttt{cutDistance\_zero\_of\_homDensity\_eq}):
    depends on both of the above.
    Used by: convergence equivalence.
\end{enumerate}
...
The graphon space is compact in the cut distance pseudometric
