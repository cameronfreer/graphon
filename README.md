...
| `Graphon/InfiniteDigraph.lean` | Directed | D1 (#84/#85): `InfiniteDigraph` as the one-sort binary R1 instance — inherits compact/standard-Borel + measure extensionality; `coordPairEquiv` (coords = ordered pairs) and the measurable `digraphEquiv` to Mathlib's `Digraph ℕ` |
