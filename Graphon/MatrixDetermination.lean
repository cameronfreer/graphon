...
private theorem starGraph_prod_eq {k : ℕ} (m : ℕ) (c : Fin k → Fin k → ℝ)
  ...
  congr 1; ext j
  have hout : s((Quot.out s((0 : Fin (m + 1)), j.succ)).1,
                (Quot.out s((0 : Fin (m + 1)), j.succ)).2) = s(0, j.succ) := Quot.out_eq _
...
private theorem doubleStarGraph_prod_eq {k : ℕ} (m p : ℕ) (c : Fin k → Fin k → ℝ)
  ...
  congr 1
  · congr 1
    · have hout : s((Quot.out s((0 : Fin (m+p+2)), ⟨1, by omega⟩)).1,
                    (Quot.out s((0 : Fin (m+p+2)), ⟨1, by omega⟩)).2) =
                  s(0, ⟨1, by omega⟩) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
      · rw [h1, h2]
      · rw [h1, h2, hc]
    · congr 1; ext j
      have hout : s((Quot.out s((0 : Fin (m+p+2)), ⟨j.val + 2, by omega⟩)).1,
                    (Quot.out s((0 : Fin (m+p+2)), ⟨j.val + 2, by omega⟩)).2) =
                  s(0, ⟨j.val + 2, by omega⟩) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
      · rw [h1, h2]
      · rw [h1, h2, hc]
  · congr 1; ext j
    have hout : s((Quot.out s((⟨1, by omega⟩ : Fin (m+p+2)), ⟨j.val + m + 2, by omega⟩)).1,
                  (Quot.out s((⟨1, by omega⟩ : Fin (m+p+2)), ⟨j.val + m + 2, by omega⟩)).2) =
                s(⟨1, by omega⟩, ⟨j.val + m + 2, by omega⟩) := Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]
    · rw [h1, h2, hc]
...
private theorem profileStarGraph_prod_eq {k : ℕ} (m r p : ℕ)
  ...
  · congr 1
    · congr 1; ext j
      have hout : s((Quot.out s((0 : Fin (m + r * (p + 1) + 1)), ⟨j.val + 1, by omega⟩)).1,
                    (Quot.out s((0 : Fin (m + r * (p + 1) + 1)), ⟨j.val + 1, by omega⟩)).2) =
                  s(0, ⟨j.val + 1, by omega⟩) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
      · rw [h1, h2]
      · rw [h1, h2, hc]
    · congr 1; ext b
      have hout : s((Quot.out s((0 : Fin (m + r * (p + 1) + 1)), profileBridgePos m p b)).1,
                    (Quot.out s((0 : Fin (m + r * (p + 1) + 1)), profileBridgePos m p b)).2) =
                  s(0, profileBridgePos m p b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
      · rw [h1, h2]
      · rw [h1, h2, hc]
  · rw [Fintype.prod_prod_type]
    ...
    have hout : s((Quot.out s(profileBridgePos m p b, profileLeafPos m p b l)).1,
                  (Quot.out s(profileBridgePos m p b, profileLeafPos m p b l)).2) =
                s(profileBridgePos m p b, profileLeafPos m p b l) := Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]
    · rw [h1, h2, hc]
...
private theorem multiProfileStarGraph_prod_eq {k : ℕ} (m L : ℕ) (r p : Fin L → ℕ)
  ...
  · congr 1
    · congr 1; ext j
      have hout : s((Quot.out s((0 : Fin (m + multiProfileBranchTotal L r p + 1)),
                                ⟨j.val + 1, by omega⟩)).1,
                    (Quot.out s((0 : Fin (m + multiProfileBranchTotal L r p + 1)),
                                ⟨j.val + 1, by omega⟩)).2) =
                  s(0, ⟨j.val + 1, by omega⟩) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    ...
    · congr 1; ext ⟨l, b⟩
      have hout : s((Quot.out s((0 : Fin (m + multiProfileBranchTotal L r p + 1)),
                                mpBridgePos m r p l b)).1,
                    (Quot.out s((0 : Fin (m + multiProfileBranchTotal L r p + 1)),
                                mpBridgePos m r p l b)).2) =
                  s(0, mpBridgePos m r p l b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
      · rw [h1, h2]
      · rw [h1, h2, hc]
  · congr 1; ext ⟨l, b, j⟩
    have hout : s((Quot.out s(mpBridgePos m r p l b, mpLeafPos m r p l b j)).1,
                  (Quot.out s(mpBridgePos m r p l b, mpLeafPos m r p l b j)).2) =
                s(mpBridgePos m r p l b, mpLeafPos m r p l b j) := Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]
    · rw [h1, h2, hc]
  ...
  have h_ind : ∀ τ : Fin n → Fin (numRowClasses c),
    ...
    · rw [if_neg h]
      obtain ⟨v, hv⟩ : ∃ v, ¬(rowClassMap c (σ v) = τ v) := by
        by_contra hall; push Not at hall; exact h (funext hall).symm
...
private def rootAttach (n : ℕ) (G : SimpleGraph (Fin (n + 1))) :
  ...
  symm.symm := fun u v h => by
    ...
    · left; exact ⟨hv, hu⟩
    · right; right; exact ⟨hv, hu, hadj.symm⟩
  loopless.irrefl := fun v h => by
    ...
    · -- Shifted G-edge
      ...
      | _ a b =>
        ...
        exact Or.inr (Or.inr ⟨by simp [Fin.val_succ], by simp [Fin.val_succ],
          by convert he' using 2 <;> simp [Fin.val_succ]⟩)
  ...
  · congr 1; ext e
    ...
    | _ a b =>
      ...
      have hout : s((Quot.out s(Fin.succ a, Fin.succ b)).1,
                    (Quot.out s(Fin.succ a, Fin.succ b)).2) = s(Fin.succ a, Fin.succ b) :=
        Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      have hout' : s((Quot.out s(a, b)).1, (Quot.out s(a, b)).2) = s(a, b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout'
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩ <;> rcases hout' with ⟨h1', h2'⟩ | ⟨h1', h2'⟩
      · rw [h1, h2, h1', h2']
      · rw [h1, h2, h1', h2', hc]
      · rw [h1, h2, h1', h2', hc]
      · rw [h1, h2, h1', h2']
...
private def rootGlue (n₁ n₂ : ℕ) (F₁ : SimpleGraph (Fin (n₁ + 1)))
  ...
  symm.symm := fun u v h => by
    ...
    · left; exact ⟨hv, hu, by convert hadj.symm using 2 <;> exact Fin.ext rfl⟩
    · right; exact ⟨hv, hu, hadj.symm⟩
  loopless.irrefl := fun v h => by
  ...
  · intro he
    ...
    | _ a b =>
      ...
      · left
        ...
        simp only [Sym2.map_mk, rootGlueEmb₁]
      ...
      · right
        refine ⟨s(⟨if a.val = 0 then 0 else a.val - n₁, by have := a.isLt; split_ifs <;> omega⟩,
                ...
                hadj, ?_⟩
        simp only [Sym2.map_mk, rootGlueEmb₂, Function.Embedding.coeFn_mk]
  ...
  · intro he
    ...
    · induction e' using Sym2.ind with
      | _ a b =>
        ...
        simp only [Sym2.map_mk, rootGlueEmb₁, SimpleGraph.mem_edgeSet,
    ...
    · induction e' using Sym2.ind with
      | _ a b =>
        simp only [SimpleGraph.mem_edgeFinset, SimpleGraph.mem_edgeSet] at he'
        simp only [Sym2.map_mk, SimpleGraph.mem_edgeFinset, SimpleGraph.mem_edgeSet]
...
private theorem rootGlue_prod_eq {k : ℕ} (n₁ n₂ : ℕ)
  ...
  have prod_map_eq : ∀ {n : ℕ} {F : SimpleGraph (Fin (n + 1))} [DecidableRel F.Adj]
    ...
    induction e using Sym2.ind with
    | _ a b =>
      simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk, Function.comp_apply]
      have hout_new : s((Quot.out (Sym2.map emb s(a, b))).1,
                        (Quot.out (Sym2.map emb s(a, b))).2) = Sym2.map emb s(a, b) :=
        Quot.out_eq _
      rw [Sym2.map_mk] at hout_new
      rw [Sym2.eq_iff] at hout_new
      have hout_old : s((Quot.out s(a, b)).1, (Quot.out s(a, b)).2) = s(a, b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout_old
      rcases hout_new with ⟨hn1, hn2⟩ | ⟨hn1, hn2⟩ <;> rcases hout_old with ⟨ho1, ho2⟩ | ⟨ho1, ho2⟩ <;> {
        rw [hn1, hn2, ho1, ho2]
        try rfl
        try exact hc _ _
      }
...
private def leftAttach2 (n : ℕ) (F : SimpleGraph (Fin (n + 2))) :
  ...
  symm.symm := fun u v h => by
    ...
    · right; left; exact ⟨hv, hu⟩
    · left; exact ⟨hv, hu⟩
    · right; right; exact ⟨b, a, hadj.symm, hb, ha⟩
  loopless.irrefl := fun v h => by
    rcases h with ⟨h1, h2⟩ | ⟨h1, h2⟩ | ⟨a, b, hadj, ha, hb⟩
    · omega
    · omega
    · have := (leftAttach2Shift n).injective (ha ▸ hb); exact F.irrefl (this ▸ hadj)
...
private theorem leftAttach2_edgeFinset (n : ℕ) (F : SimpleGraph (Fin (n + 2)))
  ...
  · intro he
    ...
    | _ a b =>
      ...
      · left; exact Sym2.eq_iff.mpr (Or.inr ⟨Fin.ext ha, Fin.ext hb⟩)
      · right
        exact ⟨s(x, y), hadj, by rw [Sym2.map_mk]; exact Sym2.eq_iff.mpr (Or.inl ⟨hx, hy⟩)⟩
  · intro he
    rcases he with rfl | ⟨e', he', rfl⟩
    · -- Bridge edge
      rw [SimpleGraph.mem_edgeSet]
...
private theorem leftAttach2_bridge_not_mem_shifted (n : ℕ)
  ...
  induction e using Sym2.ind with
  | _ a b =>
    simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk] at he
    rw [Sym2.eq_iff] at he
...
private theorem leftAttach2_prod_eq {k : ℕ} (n : ℕ) (F : SimpleGraph (Fin (n + 2)))
  ...
  congr 1
  · -- Bridge edge: resolve Quot.out
    have hout : s((Quot.out s((0 : Fin ((n + 1) + 2)), ⟨2, by omega⟩)).1,
                  (Quot.out s((0 : Fin ((n + 1) + 2)), ⟨2, by omega⟩)).2) =
                s(0, ⟨2, by omega⟩) := Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]
    · rw [h1, h2, hc]
  · congr 1; ext e
    ...
    | _ a b =>
      simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk]
      have hout : s((Quot.out s(leftAttach2Shift n a, leftAttach2Shift n b)).1,
                    (Quot.out s(leftAttach2Shift n a, leftAttach2Shift n b)).2) =
                  s(leftAttach2Shift n a, leftAttach2Shift n b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      have hout' : s((Quot.out s(a, b)).1, (Quot.out s(a, b)).2) = s(a, b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout'
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩ <;> rcases hout' with ⟨h1', h2'⟩ | ⟨h1', h2'⟩
...
private def rightAttach2 (n : ℕ) (F : SimpleGraph (Fin (n + 2))) :
  ...
  loopless.irrefl := fun v h => by
    ...
    · omega
    · omega
    · have := (rightAttach2Shift n).injective (ha ▸ hb); exact F.irrefl (this ▸ hadj)
...
private theorem rightAttach2_prod_eq {k : ℕ} (n : ℕ) (F : SimpleGraph (Fin (n + 2)))
  ...
  congr 1
  · have hout : s((Quot.out s((⟨1, by omega⟩ : Fin ((n + 1) + 2)), ⟨2, by omega⟩)).1,
                  (Quot.out s((⟨1, by omega⟩ : Fin ((n + 1) + 2)), ⟨2, by omega⟩)).2) =
                s(⟨1, by omega⟩, ⟨2, by omega⟩) := Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]
    · rw [h1, h2, hc]
  · congr 1; ext e
    ...
    | _ a b =>
      simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk]
      have hout : s((Quot.out s(rightAttach2Shift n a, rightAttach2Shift n b)).1,
                    (Quot.out s(rightAttach2Shift n a, rightAttach2Shift n b)).2) =
                  s(rightAttach2Shift n a, rightAttach2Shift n b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout
      have hout' : s((Quot.out s(a, b)).1, (Quot.out s(a, b)).2) = s(a, b) := Quot.out_eq _
      rw [Sym2.eq_iff] at hout'
      rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩ <;> rcases hout' with ⟨h1', h2'⟩ | ⟨h1', h2'⟩
...
private theorem pairOrbitRel_symm {T : ℕ} {B : Fin T → Fin T → ℝ} {W : Fin T → ℝ}
  ...
  · rw [Equiv.Perm.inv_def] at *; rw [← hw (π.symm i), Equiv.apply_symm_apply]
  · rw [Equiv.Perm.inv_def] at *; rw [← hc (π.symm i) (π.symm j)]; simp
  · rw [Equiv.Perm.inv_def, ← h1, Equiv.symm_apply_apply]
  · rw [Equiv.Perm.inv_def, ← h2, Equiv.symm_apply_apply]
...
private def eraseLabelEdge {n : ℕ} (F : SimpleGraph (Fin (n + 2))) :
  ...
  symm.symm := fun u v h => ⟨h.1.symm, by rw [Sym2.eq_swap]; exact h.2⟩
  loopless.irrefl := fun v h => F.irrefl h.1
...
private theorem labeledEval2_eraseLabelEdge {k : ℕ} (n : ℕ)
  ...
  have hlabel : c ((Fin.cons i (Fin.cons j σ) : Fin (n + 2) → Fin k)
    ...
    have hout : s((Quot.out s((0 : Fin (n + 2)), (1 : Fin (n + 2)))).1,
                  (Quot.out s((0 : Fin (n + 2)), (1 : Fin (n + 2)))).2) = s(0, 1) :=
      Quot.out_eq _
    rw [Sym2.eq_iff] at hout
    rcases hout with ⟨h1, h2⟩ | ⟨h1, h2⟩
    · rw [h1, h2]; simp [Fin.cons_zero, Fin.cons_one]
    · rw [h1, h2]; simp [Fin.cons_zero, Fin.cons_one]; exact hc j i
  rw [hlabel]; ring
...
private def edgeFreeGlue2 (n₁ n₂ : ℕ)
  ...
  symm.symm := fun u v h => by
    ...
    · left; exact ⟨b, a, hadj.symm, hb, ha⟩
    · right; exact ⟨b, a, hadj.symm, hb, ha⟩
  loopless.irrefl := fun v h => by
    rcases h with ⟨a, b, hadj, ha, hb⟩ | ⟨a, b, hadj, ha, hb⟩
    · have := (glueShift1 n₁ n₂).injective (ha ▸ hb)
      exact F₁.irrefl (this ▸ hadj)
  ...
  · intro he
    ...
    | _ a b =>
      ...
      · left; exact ⟨s(x, y), hadj,
          by simp only [Sym2.map_mk]; exact Sym2.eq_iff.mpr (Or.inl ⟨hx, hy⟩)⟩
...
private theorem tupleEquiv_restrict {T : ℕ}
  ...
  have h_edge : ∀ (ν : Fin (n + (k + 1)) → Fin T) (a b : Fin (n + (k + 1))),
    ...
    intro ν a b
    have h_out_eq : s((Quot.out (s(a, b) : Sym2 (Fin (n + (k + 1))))).1,
                      (Quot.out (s(a, b) : Sym2 (Fin (n + (k + 1))))).2) = s(a, b) :=
      Quot.out_eq _
    rcases Sym2.eq_iff.mp h_out_eq with ⟨heq1, heq2⟩ | ⟨heq1, heq2⟩
    · rw [heq1, heq2]
    · rw [heq1, heq2]; exact hB _ _
  have h_edge' : ∀ (ν : Fin (n + k) → Fin T) (a b : Fin (n + k)),
      B (ν (Quot.out (s(a, b) : Sym2 (Fin (n + k)))).1)
        (ν (Quot.out (s(a, b) : Sym2 (Fin (n + k)))).2) = B (ν a) (ν b) := by
    intro ν a b
    have h_out_eq : s((Quot.out (s(a, b) : Sym2 (Fin (n + k)))).1,
                      (Quot.out (s(a, b) : Sym2 (Fin (n + k)))).2) = s(a, b) :=
      Quot.out_eq _
    rcases Sym2.eq_iff.mp h_out_eq with ⟨heq1, heq2⟩ | ⟨heq1, heq2⟩
    · rw [heq1, heq2]
    · rw [heq1, heq2]; exact hB _ _
  ...
  · -- 3. Surjective onto F.edgeFinset.
    ...
    | _ x y =>
      ...
      -- he : ∃ a b, F'.Adj a b ∧ shift a = x ∧ shift b = y
      ...
      have hτ : ∀ v : Fin (n + k), ν (shift v) = ν' v := by
        ...
        · -- Above the pivot: shift v = v.succ, (shift v).val = v.val + 1.
          have h_shift_val : (shift v : Fin (n + (k + 1))).val = v.val + 1 := by
            ...
            · rfl
            · show p ≤ v.castSucc
              simp only [Fin.le_iff_val_le_val, Fin.val_castSucc]
              show k ≤ v.val
              omega
...
private theorem tupleEquiv_restrict_along {T k T' : ℕ}
  ...
  have h_edge' : ∀ (ν : Fin (n + T') → Fin T) (a b : Fin (n + T')),
    ...
    intro ν a b
    have h_out_eq : s((Quot.out (s(a, b) : Sym2 (Fin (n + T')))).1,
                      (Quot.out (s(a, b) : Sym2 (Fin (n + T')))).2) = s(a, b) :=
      Quot.out_eq _
    rcases Sym2.eq_iff.mp h_out_eq with ⟨heq1, heq2⟩ | ⟨heq1, heq2⟩
    · rw [heq1, heq2]
    · rw [heq1, heq2]; exact hB _ _
...
private def stripLL {n K : ℕ} (F : SimpleGraph (Fin (n + K))) :
  ...
  symm.symm := fun _ _ ⟨h1, h2⟩ => ⟨h1.symm, fun ⟨hu, hv⟩ => h2 ⟨hv, hu⟩⟩
  loopless.irrefl := fun _ ⟨h1, _⟩ => F.irrefl h1
...
private theorem labeledEvalK_glue (K : ℕ) (n₁ n₂ : ℕ)
  ...
  let F₃ : SimpleGraph (Fin ((n₁ + n₂) + K)) :=
    { Adj := fun u v =>
      ...
      symm.symm := fun u v h => by
        ...
        · left; exact ⟨hv, hu, hadj.symm⟩
        · right; exact ⟨b, a, hb, ha, hadj.symm⟩
      loopless.irrefl := fun v h => by
    ...
    · -- Forward: e ∈ F₃.edgeSet → F₁-image or F₂-image
      ...
      · -- F₁-type edge: both endpoints have val < n₁+K.
        refine Or.inl ⟨s((⟨u.val, hu⟩ : Fin (n₁+K)), (⟨v.val, hv⟩ : Fin (n₁+K))),
          hadj, ?_⟩
        simp only [Sym2.map_mk, e₁, Function.Embedding.coeFn_mk]
  ...
  have hdisj : Disjoint
    ...
    simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk, e₁, e₂,
      Function.Embedding.coeFn_mk, Sym2.eq_iff] at he₂eq
    rw [SimpleGraph.mem_edgeFinset] at he₂'
    have hadj₂ : F₂.Adj a₂ b₂ := he₂'
    have hge : ¬a₂.val < K ∨ ¬b₂.val < K := by
      by_contra h; push Not at h; exact hF₂ a₂ b₂ h.1 h.2 hadj₂
  ...
  · -- F₁ terms: B(τ₃(Quot.out(e₁.sym2Map e)...) = B(τ₁(Quot.out e)...)
    ...
    refine Sym2.ind (fun a b => ?_) e
    simp only [Function.Embedding.sym2Map_apply, Sym2.map_mk, e₁,
...
/-- **Glue witness pack**: repackages `labeledEvalK_glue`'s `Classical.choose`
result as a structure. Hides the `.choose`/`.choose_spec` chain behind
projections so downstream consumers (notably `DecLabeledGraph.mul` and
`DecLabeledGraph.eval_mul`) avoid deep `whnf` expansion during proofs. -/
private structure GluePack (K n₁ n₂ : ℕ)
    (F₁ : SimpleGraph (Fin (n₁ + K))) (F₂ : SimpleGraph (Fin (n₂ + K))) where
  graph : SimpleGraph (Fin ((n₁ + n₂) + K))
  inst : DecidableRel graph.Adj
  noLL_of : (∀ a b : Fin (n₁ + K), a.val < K → b.val < K → ¬ F₁.Adj a b) →
            ∀ a b : Fin ((n₁ + n₂) + K), a.val < K → b.val < K → ¬ graph.Adj a b
  eval_eq : ∀ {T : ℕ} (B : Fin T → Fin T → ℝ) (_hB : ∀ i j, B i j = B j i)
              (W : Fin T → ℝ) (φ : Fin K → Fin T),
            @labeledEvalK T K (n₁ + n₂) graph inst B W φ =
              @labeledEvalK T K n₁ F₁ (Classical.decRel _) B W φ *
              @labeledEvalK T K n₂ F₂ (Classical.decRel _) B W φ

/-- Package `labeledEvalK_glue`'s existential result as a `GluePack`. -/
private noncomputable def labeledEvalK_gluePack (K n₁ n₂ : ℕ)
    (F₁ : SimpleGraph (Fin (n₁ + K))) (F₂ : SimpleGraph (Fin (n₂ + K)))
    [DecidableRel F₁.Adj] [DecidableRel F₂.Adj]
    (hF₂ : ∀ a b : Fin (n₂ + K), a.val < K → b.val < K → ¬ F₂.Adj a b) :
    GluePack K n₁ n₂ F₁ F₂ :=
  let h := labeledEvalK_glue K n₁ n₂ F₁ F₂ hF₂
  { graph := h.choose
    inst := h.choose_spec.choose
    noLL_of := h.choose_spec.choose_spec.1
    eval_eq := fun B hB W φ => by
      have := h.choose_spec.choose_spec.2 B hB W φ
      -- Bridge Classical.decRel for F₁, F₂ to the ambient instances via congr.
      convert this using 2 <;> congr 1 }
...
/-! **Step 1 helper** (cycle-free base case):
...
private theorem DecLabeledGraph.trace_eval {T K n : ℕ}
  ...
  · -- hi: D.graph edges map to comap edges
    ...
    rw [SimpleGraph.mem_edgeFinset, Sym2.map_mk, SimpleGraph.mem_edgeSet]
...
This is the **single substantive step** of the
...
**Cycle check**: the natural reduction to
...
private theorem tupleEquiv_single_coord_square_moment_independent
  ...
  have hbridge := multiLabeledEvalK_tupleEquiv_invariant B hB W hW htwin M h
...
private theorem DecLabeledGraph.trace_parallel_lu0_descends {T K n : ℕ}
    ...
    { mult := fun s =>
      ...
      multNoLoop := by
        ...
        have h_not_graph : s(x, x) ∉ D.trace.graph.edgeFinset := by
          intro hmem
          rw [SimpleGraph.mem_edgeFinset, SimpleGraph.mem_edgeSet] at hmem
          exact D.trace.graph.irrefl hmem
  ...
  have hbridge := multiLabeledEvalK_tupleEquiv_invariant B hB W hW htwin M_trace h
...
Arbitrary `Dtr` can have `lu0Mult` that conflicts
    ...
    · -- **`∃ a, lu0Mult a ≥ 1`.** Further dispatch on `lu0Mult ≤ 1`.
      ...
      · -- **Sub-case `∃ a, lu0Mult a ≥ 2`.** Delegate to the named root.
        ...
        exact DecLabeledGraph.trace_parallel_lu0_descends
          D B hB W hW htwin h_diag ⟨a, ha⟩ h
  ...
  | h x y =>
    ...
    · -- Case 1: Quot.out picks (labelEmbed x, labelEmbed y).
      have eq1 : (⟨(Quot.out (Sym2.map (DecLabeledGraph.labelEmbed (n := n)) s(x, y))).1.val,
        ...
        simp only [DecLabeledGraph.labelEmbed] at this; exact this
  ...
  · intro ⟨heF, hP⟩
    ...
    · -- Show Sym2.map labelEmbed of preimage ∈ F.edgeFinset.
      have heq : Sym2.map (DecLabeledGraph.labelEmbed (n := n))
        ...
        simp only [Sym2.map_mk, DecLabeledGraph.labelEmbed,
    ...
    have hcond : (Quot.out (Sym2.map (DecLabeledGraph.labelEmbed (n := n)) a)).1.val < K ∧
      ...
      | h x y =>
        ...
        · have h1v : (Quot.out (Sym2.map (DecLabeledGraph.labelEmbed (n := n)) s(x, y))).1.val =
                     x.val := by
            have := congr_arg Fin.val h1
            simp only [Sym2.map_mk, DecLabeledGraph.labelEmbed] at this; exact this
...
/-- The `GluePack` for `D₁.mul D₂` (convenience accessor). -/
private noncomputable def DecLabeledGraph.gluePack {K n₁ n₂ : ℕ}
    (D₁ : DecLabeledGraph K n₁) (D₂ : DecLabeledGraph K n₂) :
    GluePack K n₁ n₂ D₁.graph D₂.graph :=
  letI : DecidableRel D₁.graph.Adj := Classical.decRel _
  letI : DecidableRel D₂.graph.Adj := Classical.decRel _
  labeledEvalK_gluePack K n₁ n₂ D₁.graph D₂.graph D₂.noLL

/-- **Product of decorated graphs**: glue the no-LL simple parts via the
packaged glue witness, add the LL-multiplicity maps pointwise. -/
private noncomputable def DecLabeledGraph.mul {K n₁ n₂ : ℕ}
    (D₁ : DecLabeledGraph K n₁) (D₂ : DecLabeledGraph K n₂) :
    DecLabeledGraph K (n₁ + n₂) :=
  let pack := D₁.gluePack D₂
  { graph := pack.graph
    noLL := pack.noLL_of D₁.noLL
...
private theorem DecLabeledGraph.eval_mul {T K n₁ n₂ : ℕ}
  ...
  classical
  -- Access the glue pack once; its projections avoid `Classical.choose` whnf.
  let pack := D₁.gluePack D₂
  have hmul_graph : (D₁.mul D₂).graph = pack.graph := rfl
  have hmul_llMult : (D₁.mul D₂).llMult = fun s => D₁.llMult s + D₂.llMult s := rfl
  -- Glue evaluation equality (directly from the pack).
  have hglue : @labeledEvalK T K (n₁ + n₂) pack.graph pack.inst B W φ =
               @labeledEvalK T K n₁ D₁.graph (Classical.decRel _) B W φ *
               @labeledEvalK T K n₂ D₂.graph (Classical.decRel _) B W φ :=
    pack.eval_eq B hB W φ
  -- Bridge DecidableRel instances on pack.graph (goal uses Classical.decRel).
  have bridgeF₃ : @labeledEvalK T K (n₁ + n₂) pack.graph (Classical.decRel _) B W φ =
                  @labeledEvalK T K (n₁ + n₂) pack.graph pack.inst B W φ := by
    congr 1
  have bridgeG₁ : @labeledEvalK T K n₁ D₁.graph (Classical.decRel _) B W φ =
                  labeledEvalK K n₁ D₁.graph B W φ := by congr 1
  have bridgeG₂ : @labeledEvalK T K n₂ D₂.graph (Classical.decRel _) B W φ =
                  labeledEvalK K n₂ D₂.graph B W φ := by congr 1
  -- Expand and combine.
  simp only [DecLabeledGraph.eval, hmul_graph, hmul_llMult, pow_add,
...
private theorem product_trace_identity_of_eval_tupleEquiv_invariant
  ...
  | cons p L ih =>
    ...
    · show (DecLabeledGraph.ofSimple p.2).llMult s(Fin.last k, Fin.last k) +
      ...
      simp only [Sym2.map_mk] at hmem
      exact p.2.irrefl hmem
    · intro φ
...
Infrastructure for Path (d) — the Lovász A_k algebra argument. Defines the
...
private noncomputable instance instFintypeTupleClass (T K : ℕ)
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ) :
...
private noncomputable instance instDecEqTupleClass (T K : ℕ)
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ) :
...
private noncomputable def simpleGraphEvalOn {T K : ℕ}
  ...
  Quotient.lift
    ...
    (fun ξ ξ' hξξ' => by
      ...
      intro p _
      exact @hξξ' p.1 p.2 (Classical.decRel _))
...
private lemma simpleGraphEvalOn_separates {T K : ℕ}
  ...
  | _ ξ₁ =>
    ...
    | _ ξ₂ =>
      ...
      obtain ⟨n, F, _, hF'⟩ := hne'
      -- `hF' : ¬ labeledEvalK K n F B W ξ₁ = labeledEvalK K n F B W ξ₂`
      -- (at some arbitrary DecidableRel instance that `simp` discharged).
      refine ⟨[⟨n, F⟩], ?_⟩
      -- Unfold the `simpleGraphEvalOn` value on each class via `Quotient.lift` β-reduction.
      have hL : simpleGraphEvalOn (K := K) B W [⟨n, F⟩] ⟦ξ₁⟧ =
                  @labeledEvalK T K n F (Classical.decRel _) B W ξ₁ := by
        show (([⟨n, F⟩] : List (Σ n, SimpleGraph (Fin (n + K)))).map
          (fun p => @labeledEvalK T K p.1 p.2 (Classical.decRel _) B W ξ₁)).prod = _
        simp
      have hR : simpleGraphEvalOn (K := K) B W [⟨n, F⟩] ⟦ξ₂⟧ =
                  @labeledEvalK T K n F (Classical.decRel _) B W ξ₂ := by
        show (([⟨n, F⟩] : List (Σ n, SimpleGraph (Fin (n + K)))).map
          (fun p => @labeledEvalK T K p.1 p.2 (Classical.decRel _) B W ξ₂)).prod = _
        simp
      rw [hL, hR]
      -- `hF'` asserts non-equality at SOME `DecidableRel F.Adj`. Because
      -- `labeledEvalK` is instance-independent up to definitional equality,
      -- the inequality carries across; we convert through `Classical.decRel`.
      intro heq
      apply hF'
      convert heq
...
/-- **Lagrange helper**: evaluate a list of (coefficient, list-of-graphs)
pairs as a linear combination of `simpleGraphEvalOn` products. -/
private noncomputable def coeffEval {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (coeffs : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))))
    (q : TupleClass T K B W) : ℝ :=
  (coeffs.map (fun p => p.1 * simpleGraphEvalOn B W p.2 q)).sum

private lemma coeffEval_nil {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q : TupleClass T K B W) :
    coeffEval (K := K) B W [] q = 0 := rfl

private lemma coeffEval_cons {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (c : ℝ) (L : List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))
    (rest : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))))
    (q : TupleClass T K B W) :
    coeffEval B W ((c, L) :: rest) q =
      c * simpleGraphEvalOn B W L q + coeffEval B W rest q := by
  simp [coeffEval]

private lemma coeffEval_append {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (coeffs₁ coeffs₂ : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))))
    (q : TupleClass T K B W) :
    coeffEval B W (coeffs₁ ++ coeffs₂) q =
      coeffEval B W coeffs₁ q + coeffEval B W coeffs₂ q := by
  simp [coeffEval, List.map_append, List.sum_append]

/-- **Convolution product** of two coefficient lists: pair every entry
of the first with every entry of the second, multiplying coefficients and
concatenating graph lists. -/
private def coeffMul {K : ℕ}
    (coeffs₁ coeffs₂ : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))) :
    List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))) :=
  coeffs₁.flatMap (fun p₁ => coeffs₂.map (fun p₂ => (p₁.1 * p₂.1, p₁.2 ++ p₂.2)))

private lemma coeffEval_map_mul {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (p₁ : ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))
    (coeffs₂ : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))))
    (q : TupleClass T K B W) :
    coeffEval B W (coeffs₂.map (fun p₂ => (p₁.1 * p₂.1, p₁.2 ++ p₂.2))) q =
      (p₁.1 * simpleGraphEvalOn B W p₁.2 q) * coeffEval B W coeffs₂ q := by
  induction coeffs₂ with
  | nil => simp [coeffEval]
  | cons p₂ rest ih =>
    have hmap_cons :
        (((p₁.1 * p₂.1, p₁.2 ++ p₂.2) :: rest.map (fun p₂ => (p₁.1 * p₂.1, p₁.2 ++ p₂.2))) :
          List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))) =
        (p₁.1 * p₂.1, p₁.2 ++ p₂.2) ::
          rest.map (fun p₂ => (p₁.1 * p₂.1, p₁.2 ++ p₂.2)) := rfl
    simp only [List.map_cons]
    rw [coeffEval_cons, coeffEval_cons, ih, simpleGraphEvalOn_append]
    ring

private lemma coeffEval_mul {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (coeffs₁ coeffs₂ : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))))
    (q : TupleClass T K B W) :
    coeffEval B W (coeffMul coeffs₁ coeffs₂) q =
      coeffEval B W coeffs₁ q * coeffEval B W coeffs₂ q := by
  induction coeffs₁ with
  ...
  | cons p₁ rest ih =>
    ...
    rw [coeffEval_append]
    unfold coeffMul at ih
    rw [ih]
    rw [coeffEval_map_mul]
    rw [coeffEval_cons]
    ring

/-- **Indicator data**: for each `q ≠ q₀`, pick a separating list and
record the value `simpleGraphEvalOn B W L q` that the factor will subtract. -/
private noncomputable def separatingList {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q q₀ : TupleClass T K B W) :
    List (Σ (n : ℕ), SimpleGraph (Fin (n + K))) :=
  if h : q = q₀ then []
  else Classical.choose (simpleGraphEvalOn_separates B W q q₀ h)

private lemma separatingList_spec {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q q₀ : TupleClass T K B W) (hne : q ≠ q₀) :
    simpleGraphEvalOn B W (separatingList B W q q₀) q ≠
      simpleGraphEvalOn B W (separatingList B W q q₀) q₀ := by
  unfold separatingList
  rw [dif_neg hne]
  exact Classical.choose_spec (simpleGraphEvalOn_separates B W q q₀ hne)

/-- **Parametrised indicator construction** over a list of "other" classes.
The user supplies a list `others : List TupleClass` of classes `q ≠ q₀`;
the output is a coefficient list that evaluates to `1` at `q₀` and to `0`
at any `q ∈ others` (and to a product at other points). -/
private noncomputable def indicatorCoeffsAux {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ : TupleClass T K B W)
    (others : List (TupleClass T K B W)) :
    List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))) :=
  others.foldr
    (fun q acc =>
      let L := separatingList B W q q₀
      let αq := simpleGraphEvalOn B W L q
      let αq₀ := simpleGraphEvalOn B W L q₀
      let scale := (αq₀ - αq)⁻¹
      -- Multiply acc by `scale · (L - αq · [])`.
      coeffMul [(scale, L), (-scale * αq, [])] acc)
    [(1, [])]

private lemma coeffEval_indicatorCoeffsAux_self {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ : TupleClass T K B W)
    (others : List (TupleClass T K B W))
    (hne : ∀ q ∈ others, q ≠ q₀) :
    coeffEval B W (indicatorCoeffsAux B W q₀ others) q₀ = 1 := by
  induction others with
  | nil =>
    simp [coeffEval, indicatorCoeffsAux, simpleGraphEvalOn_nil]
  | cons q rest ih =>
    have hq : q ≠ q₀ := hne q (List.mem_cons_self)
    have hrest : ∀ q' ∈ rest, q' ≠ q₀ :=
      fun q' hq' => hne q' (List.mem_cons_of_mem q hq')
    unfold indicatorCoeffsAux
    simp only [List.foldr_cons]
    rw [coeffEval_mul]
    rw [show (List.foldr _ _ rest :
        List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))) =
          indicatorCoeffsAux B W q₀ rest from rfl]
    rw [ih hrest, mul_one]
    -- Now just need the head factor evaluates to 1 at q₀.
    set L := separatingList B W q q₀ with hL
    set αq := simpleGraphEvalOn B W L q with hαq
    set αq₀ := simpleGraphEvalOn B W L q₀ with hαq₀
    set scale := (αq₀ - αq)⁻¹ with hscale
    show scale * simpleGraphEvalOn B W L q₀ +
        (-scale * αq * simpleGraphEvalOn B W [] q₀ + 0) = 1
    rw [simpleGraphEvalOn_nil]
    have hsep : αq ≠ αq₀ := by
      rw [hαq, hαq₀, hL]
      exact separatingList_spec B W q q₀ hq
    have hne' : αq₀ - αq ≠ 0 := sub_ne_zero_of_ne (Ne.symm hsep)
    have : scale * αq₀ + (-scale * αq * 1 + 0) = scale * (αq₀ - αq) := by ring
    rw [this]
    rw [hscale]
    exact inv_mul_cancel₀ hne'

private lemma coeffEval_indicatorCoeffsAux_other {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ : TupleClass T K B W)
    (others : List (TupleClass T K B W))
    (hne : ∀ q ∈ others, q ≠ q₀)
    (q : TupleClass T K B W) (hq : q ∈ others) :
    coeffEval B W (indicatorCoeffsAux B W q₀ others) q = 0 := by
  induction others with
  | nil => simp at hq
  | cons r rest ih =>
    rw [List.mem_cons] at hq
    rcases hq with hq | hq
    · -- q = r, head factor kills everything.
      subst hq
      unfold indicatorCoeffsAux
      simp only [List.foldr_cons]
      rw [coeffEval_mul]
      -- Head factor for `q` at `q = q`.
      set L := separatingList B W q q₀ with hL
      set αq := simpleGraphEvalOn B W L q with hαq
      set αq₀ := simpleGraphEvalOn B W L q₀ with hαq₀
      set scale := (αq₀ - αq)⁻¹ with hscale
      have : coeffEval B W [(scale, L), (-scale * αq, [])] q = 0 := by
        show scale * simpleGraphEvalOn B W L q +
            (-scale * αq * simpleGraphEvalOn B W [] q + 0) = 0
        rw [simpleGraphEvalOn_nil, ← hαq]
        ring
      rw [this, zero_mul]
    · -- q ∈ rest, use IH.
      have hrest : ∀ q' ∈ rest, q' ≠ q₀ :=
        fun q' hq' => hne q' (List.mem_cons_of_mem r hq')
      unfold indicatorCoeffsAux
      simp only [List.foldr_cons]
      rw [coeffEval_mul]
      rw [show (List.foldr _ _ rest :
          List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K))))) =
            indicatorCoeffsAux B W q₀ rest from rfl]
      rw [ih hrest hq, mul_zero]

/-- **Indicator coefficients for `q₀`**: over all other classes. -/
private noncomputable def indicatorCoeffs {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ : TupleClass T K B W) :
    List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))) :=
  indicatorCoeffsAux B W q₀ (Finset.univ.erase q₀).toList

private lemma coeffEval_indicatorCoeffs_self {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ : TupleClass T K B W) :
    coeffEval B W (indicatorCoeffs B W q₀) q₀ = 1 := by
  unfold indicatorCoeffs
  apply coeffEval_indicatorCoeffsAux_self
  intro q hq
  simp at hq
  exact hq

private lemma coeffEval_indicatorCoeffs_other {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
    (q₀ q : TupleClass T K B W) (hne : q ≠ q₀) :
    coeffEval B W (indicatorCoeffs B W q₀) q = 0 := by
  unfold indicatorCoeffs
  apply coeffEval_indicatorCoeffsAux_other
  · intro q' hq'
    simp at hq'
    exact hq'
  · simp
...
/-- **Scaled indicator**: each entry of the indicator list, scaled by `c`. -/
private noncomputable def scaledIndicatorCoeffs {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ) (c : ℝ)
    (q₀ : TupleClass T K B W) :
    List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + K)))) :=
  (indicatorCoeffs B W q₀).map (fun p => (c * p.1, p.2))

private lemma coeffEval_scaledIndicatorCoeffs {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ) (c : ℝ)
    (q₀ q : TupleClass T K B W) :
    coeffEval B W (scaledIndicatorCoeffs B W c q₀) q =
      c * coeffEval B W (indicatorCoeffs B W q₀) q := by
  unfold scaledIndicatorCoeffs coeffEval
  induction indicatorCoeffs B W q₀ with
  | nil => simp
  | cons p rest ih =>
...
The coefficient list is obtained by Lagrange interpolation: for each
class `q₀`, we scale the indicator `1_{q₀}` by `g q₀` and concatenate
over all `q₀`. -/
private theorem simpleGraphEvalOn_spans {T K : ℕ}
  ...
  -- Build `coeffs` as the concatenation of `scaledIndicatorCoeffs B W (g q₀) q₀`
  -- over all `q₀ : TupleClass`.
  classical
  refine ⟨(Finset.univ : Finset (TupleClass T K B W)).toList.flatMap
    (fun q₀ => scaledIndicatorCoeffs B W (g q₀) q₀), ?_⟩
  intro q
  -- Reduce the sum over the flatMap to a `coeffEval` over the flatMap,
  -- then to a sum over q₀ of `coeffEval (scaledIndicatorCoeffs B W (g q₀) q₀)`.
  show g q = coeffEval B W _ q
  -- Generalize: for any list of `q₀` values, `coeffEval (flatMap ...) q`
  -- equals `∑ q₀ ∈ list, g q₀ * coeffEval (indicatorCoeffs B W q₀) q`.
  have hflat :
      ∀ (l : List (TupleClass T K B W)),
        coeffEval B W (l.flatMap (fun q₀ => scaledIndicatorCoeffs B W (g q₀) q₀)) q =
          (l.map (fun q₀ => g q₀ * coeffEval B W (indicatorCoeffs B W q₀) q)).sum := by
    intro l
    induction l with
    | nil => simp [coeffEval]
    | cons q₀ rest ih =>
      simp only [List.flatMap_cons, List.map_cons, List.sum_cons]
      rw [coeffEval_append, ih, coeffEval_scaledIndicatorCoeffs]
  rw [hflat]
  -- Now reduce the list sum to a finset sum over `Finset.univ`.
  have hlist_sum :
      ((Finset.univ : Finset (TupleClass T K B W)).toList.map
        (fun q₀ => g q₀ * coeffEval B W (indicatorCoeffs B W q₀) q)).sum =
      ∑ q₀ : TupleClass T K B W, g q₀ * coeffEval B W (indicatorCoeffs B W q₀) q := by
    simp
  rw [hlist_sum]
  -- Kronecker delta: `coeffEval B W (indicatorCoeffs B W q₀) q` = `if q = q₀ then 1 else 0`.
  have hindic :
      ∀ q₀ : TupleClass T K B W,
        coeffEval B W (indicatorCoeffs B W q₀) q =
          if q = q₀ then 1 else 0 := by
    intro q₀
    by_cases h : q = q₀
    · subst h
      simp [coeffEval_indicatorCoeffs_self]
    · simp [h, coeffEval_indicatorCoeffs_other B W q₀ q h]
  simp only [hindic, mul_ite, mul_one, mul_zero]
  simp only [show ∀ x : TupleClass T K B W, (q = x) ↔ (x = q) from fun x => eq_comm]
...
Since the cycle among the three interface theorems is semantic (see above),
the content is captured by a single named theorem. The A_k
...
— it lands in `TupleClass T K B W → ℝ` iff `tr_k_descends_to_A_k` holds. -/
private noncomputable def tr_k {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (W : Fin T → ℝ)
...
private theorem tr_k_singleton_descends {T K : ℕ}
  ...
  let G : SimpleGraph (Fin ((n + 1) + K)) :=
    { Adj := fun u v => F.Adj ⟨u.val, by have := u.isLt; omega⟩
      ...
      loopless.irrefl := fun _ h => F.irrefl h }
  ...
  have bridge : ∀ φ : Fin K → Fin T,
    ...
    · intro a ha
      refine Sym2.ind (fun u v _ => ?_) a ha
      simp only [Sym2.map_mk]
      have hout_F : Quot.out s(u, v) = (u, v) ∨ Quot.out s(u, v) = (v, u) :=
        Sym2.rel_iff'.mp (Sym2.Rel.is_equivalence.eqvGen_iff.mp
          (Quot.eqvGen_exact (Quot.out_eq s(u, v))))
      have hout_G : Quot.out s(e u, e v) = (e u, e v) ∨ Quot.out s(e u, e v) = (e v, e u) :=
        Sym2.rel_iff'.mp (Sym2.Rel.is_equivalence.eqvGen_iff.mp
          (Quot.eqvGen_exact (Quot.out_eq s(e u, e v))))
...
Paper-faithful framing: introduce the class-function algebra on
`TupleClass T K B W` and name the algebraic content after Lovász's
connection-matrix trace stability theorem.
...
**Why the obvious reduction is circular.** The obvious reduction chain
```
...
Build the closure of `weightedInnerProduct_descends`
...
private theorem exists_decGraph_for_connCol {T K : ℕ}
  ...
  | cons p rest ih =>
    ...
    · -- llMult on every diagonal pair `s(x, x)` is zero, by induction.
      ...
      rw [if_neg]
      intro h
      rw [Sym2.map_mk, SimpleGraph.mem_edgeFinset, SimpleGraph.mem_edgeSet] at h
      exact p.2.irrefl h
...
private theorem weightedInnerProduct_descends {T K : ℕ}
  ...
  exact DecLabeledGraph.trace_eval_tupleEquiv_invariant D B hB W hW htwin h_diag h
...
private theorem Ak_trace_stable_generators {T K : ℕ}
  ...
  have reshape : ∀ ζ : Fin K → Fin T,
    ...
    ring
  rw [reshape ξ, reshape ξ']
  exact weightedInnerProduct_descends B hB W hW htwin L₁ L₂ h
  ...
  have inner : ∀ (L₁ : List (Σ (n : ℕ), SimpleGraph (Fin (n + (K + 1)))))
    ...
    | cons pG restG ihG =>
      ...
      rw [tr_k_smul, tr_k_smul,
          Ak_trace_stable_generators B hB W hW htwin L₁ pG.2 h]
...
The mathematical content lives entirely in
...
private theorem tr_k_descends_to_A_k {T K : ℕ}
  ...
  | cons p rest ih =>
    ...
    have h_gen : tr_k B W (fun q => p.1 * simpleGraphEvalOn B W p.2 q) ξ =
      ...
      rw [tr_k_smul, tr_k_smul, tr_k_generator_descends B hB W hW htwin p.2 h]
    rw [h_gen]
...
**CANONICAL ROOT**: this theorem is equivalent to `tr_k_descends_to_A_k` (see the A_k/tr_k
section above) — namely, `traceMeasure ξ = traceMeasure ξ'` ⟺ `tr_k`
descends for every `g`, by pairing against the `simpleGraphEvalOn`
  ...
  have hswap_list : ∀ (cs : List (ℝ × List (Σ (n : ℕ), SimpleGraph (Fin (n + (K + 1)))))),
    ...
    | cons p rest IH =>
      ...
      have h_factor_gen :
        ...
        have hgen := traceMeasure_simpleGraphEvalOn_eq_of_tupleEquiv B hB W hW htwin h p.2
...
**CANONICAL ROOT**: this theorem is `tr_k_descends_to_A_k` applied to
`g := starKernelClass B hB W m`
(which is independently class-invariant via `starKernel_tupleEquiv_invariant`),
together with the pushforward identity
`starMultigraphEval B W ξ m = tr_k B W starKernelClass ⟦ξ⟧`. The current
proof is written via `traceMeasure_eq_of_tupleEquiv` → (through the
cycle) — same content, different name. -/
private theorem starMultigraphEval_tupleEquiv_invariant_direct {T K : ℕ}
    (B : Fin T → Fin T → ℝ) (hB : ∀ i j, B i j = B j i) (W : Fin T → ℝ)
    (hW : ∀ i, 0 < W i) (htwin : ∀ i j, i ≠ j → B i ≠ B j) (m : Fin K → ℕ)
    {ξ ξ' : Fin K → Fin T} (h : tupleEquiv B W ξ ξ') :
  ...
  rw [hpush ξ, hpush ξ', traceMeasure_eq_of_tupleEquiv B hB W hW htwin h]
...
private theorem starMultigraphEval_tupleEquiv_invariant_direct_K1 {T : ℕ}
  ...
  have hstar := starMultigraphEval_tupleEquiv_invariant_direct B hB W hW htwin
    (fun _ : Fin 1 => r) h
...
private theorem starMultigraphEval_in_simpleGraphSpan {T K : ℕ}
  ...
  classical
  -- Descend `starMultigraphEval B W · m` to the quotient using the
  -- direct invariance frontier.
  let g : TupleClass T K B W → ℝ :=
    Quotient.lift (fun ξ => starMultigraphEval B W ξ m)
      (fun ξ ξ' hξξ' =>
        starMultigraphEval_tupleEquiv_invariant_direct B hB W hW htwin m hξξ')
  -- Apply Step 4 (quotient-level fullness) to get coefficients.
  obtain ⟨coeffs, hcoeffs⟩ := simpleGraphEvalOn_spans B W g
  refine ⟨coeffs, fun ξ => ?_⟩
  -- `g ⟦ξ⟧ = starMultigraphEval B W ξ m` by Quotient.lift_mk.
  have hg_ξ : g ⟦ξ⟧ = starMultigraphEval B W ξ m := rfl
  rw [← hg_ξ, hcoeffs ⟦ξ⟧]
  -- `simpleGraphEvalOn B W p.2 ⟦ξ⟧ = labeledEvalK ... ξ`-product by definition.
  rfl

/-- **Corollary of the span representation** — kept for downstream
...
private theorem starMultigraphEval_tupleEquiv_invariant {T K : ℕ}
  ...
  starMultigraphEval_tupleEquiv_invariant_direct B hB W hW htwin m h
...
**Conclusion:** strategies (a) and (b) fail geometrically; (c) fails at the
weight-integrality step; (d) is the only guaranteed route and requires the
A_k infrastructure (finite-dimensionality, polynomial identities, multigraph
span membership). The proof below routes through the star-multigraph
theorem. -/
private theorem tupleEquiv_single_coord_square_moment {T K : ℕ}
  ...
  have hprod : ∀ (ξ₀ : Fin K → Fin T) (t : Fin T),
    ...
    simp [m]
  have hstar := starMultigraphEval_tupleEquiv_invariant B hB W hW htwin m h
  unfold starMultigraphEval at hstar
...
```
∀ (m : Fin K → ℕ), ∑ t : Fin T, W t * ∏ a : Fin K, B (ξ a) t ^ m a
                = ∑ t : Fin T, W t * ∏ a : Fin K, B (ξ' a) t ^ m a
```

**Why this is the frontier in its simplest form.** When `m` is a
0/1-indicator, this is a standard simple-graph labeledEvalK (a "star"
centered on an unlabeled vertex), so follows from simple-graph
`tupleEquiv`. For `m ≥ 2` on some coordinate, this is the first case
...
`starMultigraphEval_tupleEquiv_invariant`):
  - Newton–Girard interchange between power sums and elementary
    ...
    tupleEquiv-invariant (as labeledEvalK of appropriate
    distinct-neighbor graphs with inclusion-exclusion). If we can
    bound the moment degree by `T` (automatic by finiteness) and
    recover power sums from elementary symmetric polynomials, we
    get the full invariance.
  - Polynomial identity via Lovász's A_k algebra
    (TR-2004-82 §3–4): the algebra generated by
    `{labeledEvalK F B W · : F simple}` closes under multiplication
...
/-! #### Level 4 — Claim 4.3 / 4.4 via extension
...
the non-surjective branch closes by:
  ...
  3. Surjective case via `tupleEquiv_surjective_case_ext`: does not
     require out-of-range IH (unlike the existing
...
statements reduce to it by routine assembly modulo the extension-form
variants at Level 4. -/
...
**Proof path**: thin wrapper around `tr_k_generator_descends` via unfolding
`tr_k` and `simpleGraphEvalOn`. (Routing instead through
`product_trace_identity_of_eval_tupleEquiv_invariant` with
`Dtr.eval_tupleEquiv_invariant` as hypothesis would create the semantic
cycle described above; the canonical root `tr_k_generator_descends`
avoids it.) -/
private theorem product_trace_identity {T : ℕ}
    (B : Fin T → Fin T → ℝ) (hB : ∀ i j, B i j = B j i) (W : Fin T → ℝ)
    (hW : ∀ i, 0 < W i) (htwin : ∀ i j, i ≠ j → B i ≠ B j) {k : ℕ}
    {ξ ξ' : Fin k → Fin T} (h : tupleEquiv B W ξ ξ')
    (L : List (Σ (n : ℕ), SimpleGraph (Fin (n + (k + 1))))) :
    ∑ t : Fin T, W t *
      (L.map (fun p =>
...
private theorem coeffRestrict_equiv {T : ℕ}
  ...
  have hd_zero : ∀ q, d_fun q = 0 := by
    ...
    · -- hortho: via `product_trace_identity`.
      ...
      linarith [product_trace_identity B hB W hW htwin h L]
...
private theorem tupleEquiv_extend {T : ℕ}
  ...
  have hpos_ψ : 0 < coeffRestrict B W μ ψ := by
    rwa [coeffRestrict_equiv B W hW hB htwin μ (hμ ▸ h)] at hpos
  -- Extract a witness.
