...
/-!
# Purely spare screening: the finite-cylinder identity
...
The **finite-cylinder identity** combines the two. For any measurable observation `ζ` of the
latent array which reads at least the local latent window at `A`, the conditional probability
given `ζ` of an `A`-cylinder meeting a `B`-cylinder and a whole-latent event factors as the
product of the conditional probabilities given `ζ`.

In the pooled reading, `A` is a finite original support, `B` a finite spare support, and `ζ`
the restriction of the pooled old latents to the original vertices. The identity is the cylinder
step of the screening
statement "the original structure is conditionally independent of the purely spare structure
and the whole pooled latent array, given the original old latents". The extension from
cylinders to the full observations is not part of this module.

The identity concerns cylinders of the purely spare structure only. It says nothing about
mixed observations, and the conditioning observation `ζ` is never replaced by a coarser one.
-/
